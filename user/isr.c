...
IFX_INTERRUPT(uart2_rx_isr, 0, UART2_RX_INT_PRIO)
{
   ...
   if(gnss.latitude == 0.0f && gnss.longitude == 0.0f)
   {
       return;
   }

   if (dot_count < 20)                             //记录20次点
   {
       dot_count++;
       Start_Latitude += gnss.latitude;
       Start_Longitude += gnss.longitude;
       Start_Direaction += gnss.direction;
       gnss_state = 0;
   }
   else if (dot_count == 20)                       //平均得起点，耗时2s
   {
       dot_count++;
       Start_Latitude /= 20;
       Start_Longitude /= 20;
   ...
   Distance = get_two_points_distance(Start_Latitude, Start_Longitude, gnss.latitude, gnss.longitude);        //偏离起点距离
   Azimuth  = get_two_points_azimuth(Start_Latitude, Start_Longitude, gnss.latitude, gnss.longitude);         //偏离起点角度
