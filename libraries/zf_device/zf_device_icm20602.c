...
//// 参数说明     void
//// 返回参数     void
//// 使用示例     icm20602_data_trans();                                            // 执行该函数后，直接查看对应的变量即可
//// 备注信息     单位转化成g和rad/s
////           坐标系转变为右前上
////-------------------------------------------------------------------------------------------------------------------
//void icm20602_data_trans(void)
//{
//    IMUData.ax_RowFrame = icm20602_acc_transition(icm20602_acc_x);
//    IMUData.ay_RowFrame = icm20602_acc_transition(icm20602_acc_y);
//    IMUData.az_RowFrame = icm20602_acc_transition(icm20602_acc_z);
//
//    IMUData.gx_RowFrame = Deg2Rad(icm20602_gyro_transition(icm20602_gyro_x));
//    IMUData.gy_RowFrame = Deg2Rad(icm20602_gyro_transition(icm20602_gyro_y));
...
//    IMUData.gy -= IMUData.gyOffset;
//    IMUData.gz -= IMUData.gzOffset;
//}
//
////-------------------------------------------------------------------------------------------------------------------
//// 函数简介     计算icm2060陀螺仪零漂数据
//// 参数说明     void
//// 返回参数     void
//// 使用示例     icm20602_offset();                                            // 执行该函数后，直接查看对应的变量即可
//// 备注信息     单位为rad/s，增加了扰动监测及计数，每2ms读值一次，共读1000次
////-------------------------------------------------------------------------------------------------------------------
...
//    IMUData.gzOffset = 0;
//
//    IMUData.gxOffsetSum = 0;
//    IMUData.gyOffsetSum = 0;
//    IMUData.gzOffsetSum = 0;
//
//
//    IMUData.offsetErrCount = 0;
//    OLED_CLS(); //清屏
//
//    for(uint32 i = 0;i < 1000;i++)
//    {
//        icm20602_get_gyro();
//        icm20602_data_trans();//数据转为弧度制后再进行除零飘
//        if(ABS(IMUData.gx) < 0.05 && ABS(IMUData.gy) < 0.05 && ABS(IMUData.gz) < 0.05)//防止车体晃动干扰
//        {
//            IMUData.gxOffsetSum += IMUData.gx;
//            IMUData.gyOffsetSum += IMUData.gy;
//            IMUData.gzOffsetSum += IMUData.gz;
//        }
//        else//车体检测到晃动干扰 读值不加到和上，并且重新计数
//        {
//            i--;
//            IMUData.offsetErrCount++;
//        }
//        OLED_Show_Numbers(First_X,4,i,TextSize_F6x8);
//        system_delay_ms(2);
//    }
//    IMUData.gxOffset = IMUData.gxOffsetSum/1000;
//    IMUData.gyOffset = IMUData.gyOffsetSum/1000;
//    IMUData.gzOffset = IMUData.gzOffsetSum/1000;
//}
//
////-------------------------------------------------------------------------------------------------------------------
//// 函数简介     初始化 ICM20602
...
//        // 设置为 0x00 陀螺仪量程为 ±250  dps    获取到的陀螺仪数据除以 131.072   可以转化为带物理单位的数据 单位为 °/s
//        // 设置为 0x08 陀螺仪量程为 ±500  dps    获取到的陀螺仪数据除以 65.536    可以转化为带物理单位的数据 单位为 °/s
//        // 设置为 0x10 陀螺仪量程为 ±1000 dps    获取到的陀螺仪数据除以 32.768    可以转化为带物理单位的数据 单位为 °/s
//        // 设置为 0x18 陀螺仪量程为 ±2000 dps    获取到的陀螺仪数据除以 16.384    可以转化为带物理单位的数据 单位为 °/s
//        switch(ICM20602_GYRO_SAMPLE_DEFAULT)
//        {
//            case ICM20602_GYRO_SAMPLE_SGN_250DPS:
//            {
//                icm20602_write_register(ICM20602_GYRO_CONFIG, 0x00);
//                icm20602_transition_factor[1] = 131.072;
//            }break;
//            case ICM20602_GYRO_SAMPLE_SGN_500DPS:
//            {
//                icm20602_write_register(ICM20602_GYRO_CONFIG, 0x08);
//                icm20602_transition_factor[1] = 65.536;
//            }break;
//            case ICM20602_GYRO_SAMPLE_SGN_1000DPS:
//            {
//                icm20602_write_register(ICM20602_GYRO_CONFIG, 0x10);
//                icm20602_transition_factor[1] = 32.768/Temperature;
//            }break;
//            case ICM20602_GYRO_SAMPLE_SGN_2000DPS:
//            {
//                icm20602_write_register(ICM20602_GYRO_CONFIG, 0x18);
//                icm20602_transition_factor[1] = 16.384;
//            }break;
//            default:
//            {
