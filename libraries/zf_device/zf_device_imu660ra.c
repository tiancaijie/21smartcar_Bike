...
//
//int16 imu660ra_gyro_x = 0, imu660ra_gyro_y = 0, imu660ra_gyro_z = 0;            // 三轴陀螺仪数据原值      GYRO (陀螺仪)
//int16 imu660ra_acc_x = 0, imu660ra_acc_y = 0, imu660ra_acc_z = 0;               // 三轴加速度计数据     ACC  (accelerometer 加速度计)
//float imu660ra_ax = 0,imu660ra_ay = 0,imu660ra_az = 0;
//float imu660ra_gx = 0,imu660ra_gy = 0,imu660ra_gz = 0;                          //国际单位
//float imu660ra_transition_factor[2] = {4096, 16.4};                             // 转换实际值的比例
////#define temperature (1+0.008464)  //温度补偿系数2024.1.13
////#define temperature (1+0.013768)  //温度补偿系数2024.4.14
////#define temperature (1.00526370)  //温度补偿系数2024.6.15
//#define temperature (1.001688099)  //温度补偿系数2024.6.15
//static const unsigned char imu660ra_config_file[8192] = {
...
//// 参数说明     void
//// 返回参数     void
//// 使用示例     imu660ra_data_trans();                                            // 执行该函数后，直接查看对应的变量即可
//// 备注信息     单位转化成g和rad/s
////           坐标系转变为右前上
////-------------------------------------------------------------------------------------------------------------------
//
//void imu660ra_data_trans(void)
//{
//    imu660ra_ax = imu660ra_acc_x / imu660ra_transition_factor[0];
//    imu660ra_ay = imu660ra_acc_y / imu660ra_transition_factor[0];
//    imu660ra_az = imu660ra_acc_z / imu660ra_transition_factor[0];
//
//    imu660ra_gx = imu660ra_gyro_x / imu660ra_transition_factor[1] * (float)0.0174532925199;
//    imu660ra_gy = imu660ra_gyro_y / imu660ra_transition_factor[1] * (float)0.0174532925199;
//    imu660ra_gz = imu660ra_gyro_z / imu660ra_transition_factor[1] * (float)0.0174532925199;
//
//    IMUData.ax =  imu660ra_ay;
//    IMUData.ay = -imu660ra_ax;
//    IMUData.az =  imu660ra_az;
//
//    IMUData.gx =  imu660ra_gy;
//    IMUData.gy = -imu660ra_gx;
//    IMUData.gz =  imu660ra_gz;
//
////    IMUData.gx -= IMUData.gxOffset;
////    IMUData.gy -= IMUData.gyOffset;
////    IMUData.gz -= IMUData.gzOffset;
//    IMUData.gx -= -0.0020828;
//    IMUData.gy -=  0.0015729;
//    IMUData.gz -=  0.0022867;
//
//}
////共10ms
////-------------------------------------------------------------------------------------------------------------------
//// 函数简介     计算IMU660RA 陀螺仪零漂数据
//// 参数说明     void
//// 返回参数     void
//// 使用示例     imu660ra_offset();                                            // 执行该函数后，直接查看对应的变量即可
//// 备注信息     单位为rad/s，增加了扰动监测及计数，每2ms读值一次，共读1000次
////-------------------------------------------------------------------------------------------------------------------
//void imu660ra_offset(void)
//{
//    IMUData.gx = 0;
//    IMUData.gy = 0;
//    IMUData.gz = 0;
...
//    {
//        imu660ra_get_gyro();
//        imu660ra_data_trans();//数据转为弧度制后再进行除零飘
...
//        system_delay_ms(2);
//    }
//    IMUData.gxOffset = IMUData.gxOffsetSum/10000;
//    IMUData.gyOffset = IMUData.gyOffsetSum/10000;
//    IMUData.gzOffset = IMUData.gzOffsetSum/10000;
//
//
//
//
...
//        // 设置为:0x04 陀螺仪量程为:±125 dps      获取到的陀螺仪数据除以262.144         可以转化为带物理单位的数据，单位为：°/s
...
//                imu660ra_transition_factor[1] = 65.536/temperature;
//            }break;
