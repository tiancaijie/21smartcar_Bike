...
// 参数说明     void
// 返回参数     void
// 使用示例     imu963_data_trans();                                            // 执行该函数后，直接查看对应的变量即可
// 备注信息     单位转化成g和rad/s
//           坐标系转变为右前上
//-------------------------------------------------------------------------------------------------------------------
void imu963_data_trans(void)
{
    IMUData.ax_RowFrame = imu963ra_acc_transition(imu963ra_acc_x);
    IMUData.ay_RowFrame = imu963ra_acc_transition(imu963ra_acc_y);
    IMUData.az_RowFrame = imu963ra_acc_transition(imu963ra_acc_z);;

    IMUData.gx_RowFrame = Deg2Rad(imu963ra_gyro_transition(imu963ra_gyro_x));
