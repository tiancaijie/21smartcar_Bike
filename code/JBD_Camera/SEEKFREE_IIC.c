...
#define SDA_INIT     gpio_init(SEEKFREE_SDA, GPO, 1, GPO_OPEN_DTAIN)   //IIC的SDA
#define SCL_INIT     gpio_init(SEEKFREE_SCL, GPO, 1, GPO_PUSH_PULL)    //IIC的SCL
...
#define DIR_IN()        gpio_set_dir  (SEEKFREE_SDA, GPI, GPI_PULL_UP)    //输入方向
...
//内部使用，用户无需调用
void simiic_start(void) //IIC_start
{
	SDA1();
	SCL1();
	simiic_delay();
...
//内部使用，用户无需调用
void simiic_stop(void) //IIC_stop
{
	SDA0();
	SCL0();
	simiic_delay();
...
//内部使用，用户无需调用
void simiic_sendack(unsigned char ack_dat) // I2C_SendACK
{
    SCL0();
	simiic_delay();
	if(ack_dat) SDA0();
...
static int sccb_waitack(void) // SCCB_WaitAck
{
    SCL0();
	DIR_IN();
	simiic_delay();
...
//16字节发送程序
void send16_ch(uint16 c)
{
    uint8 i = 8;
    uint8 u = 8;//输出低八位
    while(i--)//输出高八位
    {
        if(c & 0x8000)  SDA1();//SDA 输出数据
        else            SDA0();
        c <<= 1;
        simiic_delay();
        SCL1();                //SCL 拉高，采集信号
...
//  Sample usage:				
//-------------------------------------------------------------------------------------------------------------------
void simiic_write_reg(uint8 dev_add, uint8 reg, uint8 dat) //simiic_write_reg
{
	simiic_start();
...
//模拟IIC写16位数据到设备寄存器函数
void simiic_write16_reg(uint8 dev_add, uint8 reg, uint16 dat)
{
    simiic_start();
    send_ch( (dev_add<<1) | 0x00);               //发送器件地址加写位
    send_ch( reg );                  //发送从机寄存器地址
    send16_ch( dat );                //发送需要写入的数据
    simiic_stop();
}
...
//  Sample usage:				
//-------------------------------------------------------------------------------------------------------------------
uint8 simiic_read_reg(uint8 dev_add, uint8 reg, IIC_type type) //simiic_read_reg
{
	uint8 dat;
	simiic_start();
    send_ch( (dev_add<<1) | 0x00);  //发送器件地址加写位
...
//模拟IIC从设备寄存器读取16位数据
uint16 simiic_read16_reg(uint8 dev_add, uint8 reg, IIC_type type)
{
    uint16 dat;
    simiic_start();
    send_ch( (dev_add<<1) | 0x00);              //发送器件地址加写位
